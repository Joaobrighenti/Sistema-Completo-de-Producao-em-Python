...
def gera_cabecalho_retirada():
    cabecalho =  dbc.Card(
        ...
        [
            ...
            dbc.Col("QTD RET", style={'font-weight': 'bold', 'text-align': 'center', 'flex': '0 0 7%'}), 
...
def gerar_cards_retirada(df, cliente, pcp, produto, ano, semana, categoria):
    df = pd.DataFrame(df)

    df['data_retirada'] = pd.to_datetime(df['data_retirada'], errors='coerce')  # Converte para datetime
...
def gerar_card_movimentacao(df):
    ...
    df['qtd_retirada'] = f"{int(df['qtd_retirada']):,}".replace(",", ".")
    card = dbc.Card(
    [
        dbc.Row(
            [
                ...
                dbc.Col(
                    dbc.Button(
                        ...
                        size='xs',
                        outline=True,
                        id={'type': 'editar_reti', 'index': df['id_retirada']}
                    ),
                ...
                dbc.Col(
                    dbc.Button(
                        ...
                        size='xs',
                        outline=True,
                        id={'type': 'deletar_reti', 'index': df['id_retirada']},
                    ),
                    style={'flex': '0 0 3%', 'height': '35px', 'text-align': 'center', 'background': 'white', 'display': 'flex', 'justify-content': 'center', 'align-items': 'center'},
                ),
                ...
                dbc.Col(f"{df['qtd_retirada']}", style={'flex': '0 0 7%', 'height': '35px', 'text-align': 'center', 'background': '#0a385c', 'color':'white', 'display': 'flex', 'justify-content': 'center', 'align-items': 'center','fontWeight': 'bold'}),
                dbc.Col(f"{df['observacao']}", style={'flex': '0 0 30%', 'height': '35px', 'text-align': 'center', 'background': 'white', 'display': 'flex', 'justify-content': 'center', 'align-items': 'center'}),
                dbc.Col(f"{(df['data_retirada'])}", style={'flex': '0 0 7%', 'height': '35px', 'text-align': 'center', 'background': 'white', 'display': 'flex', 'justify-content': 'center', 'align-items': 'center'})
            ],
            style={
                'display': 'flex',
